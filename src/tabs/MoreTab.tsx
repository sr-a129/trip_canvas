...
import { ArrowLeft, Check, ExternalLink, Wifi, Car, X, Plus } from 'lucide-react'
import type { Trip, AppMode, MemoItem, PackingItem } from '../types'
...
type SubScreen = 'menu' | 'flight' | 'hotel' | 'memo' | 'packing'
    ...
    <div className="px-5 py-4 space-y-4">
      {trip.flights.map((f) => (
        <div key={f.flightNumber} className="bg-white rounded-2xl overflow-hidden shadow-sm">
          ...
          >
            <span className="text-white text-lg">{f.type === 'departure' ? '🛫' : '🛬'}</span>
            ...
            <div className="grid grid-cols-2 gap-3">
              {f.terminal && (
                <div className="bg-cream rounded-xl p-3">
                  <p className="text-muted text-[10px] font-medium mb-0.5">ターミナル</p>
                  <p className="text-navy text-sm font-semibold">{f.terminal}</p>
                </div>
              )}
              {f.gate && (
                <div className="bg-cream rounded-xl p-3">
                  <p className="text-muted text-[10px] font-medium mb-0.5">ゲート</p>
      ...
      <div className="bg-white rounded-2xl overflow-hidden shadow-sm">
        ...
        <div className="relative h-40 bg-navy">
          ...
          <div className="absolute bottom-4 left-5">
            <h3 className="text-white font-bold text-xl" style={{ fontFamily: 'Playfair Display, Georgia, serif' }}>
              {h.name}
            </h3>
        ...
        <div className="p-4 space-y-3">
          <div className="text-muted text-xs font-medium" style={{ fontFamily: 'DM Mono, monospace' }}>
            {nights} 泊
          </div>
          ...
          {h.wifi && (
            <div className="flex items-start gap-3 bg-cream rounded-xl p-3">
              ...
              <div>
                ...
                <p className="text-navy text-sm mt-0.5" style={{ fontFamily: 'DM Mono, monospace' }}>
                  {h.wifi}
                </p>
              </div>
            </div>
          ...
          {h.keyCode && (
            <div className="flex items-start gap-3 bg-cream rounded-xl p-3">
              ...
              <div>
                ...
                <p className="text-navy text-sm font-bold mt-0.5">{h.keyCode}</p>
              </div>
            </div>
          )}
          ...
          {h.notes && (
            <div className="bg-cream rounded-xl p-3">
              <p className="text-muted text-[10px] font-medium mb-1">施設ルール</p>
              <p className="text-navy text-sm leading-relaxed whitespace-pre-line">{h.notes}</p>
            </div>
          )}
...
function MemoScreen({ trip, mode }: { trip: Trip; mode: AppMode }) {
  ...
  const toggleChecklistItem = (memoId: string, itemIdx: number) => {
    setItems((prev) =>
      prev.map((m) =>
        m.id === memoId && m.items
          ? { ...m, items: m.items.map((it, i) => (i === itemIdx ? { ...it, done: !it.done } : it)) }
          : m
      )
    )
  }
      ...
      {[...pinned, ...unpinned].map((memo) => (
        <div key={memo.id} className="bg-white rounded-2xl p-4 shadow-sm">
          ...
          {memo.type === 'note' && memo.content && (
            <p className="text-muted text-sm leading-relaxed whitespace-pre-line">{memo.content}</p>
          )}
          ...
          {memo.type === 'checklist' && memo.items && (
            <div className="space-y-2">
              {memo.items.map((item, idx) => (
                <button
                  key={idx}
                  onClick={() => mode === 'host' && toggleChecklistItem(memo.id, idx)}
                  className="w-full flex items-center gap-3 text-left"
                >
                  <div
                    className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all ${
                      item.done ? 'bg-teal border-teal' : 'border-bdr'
                    }`}
                  >
                    {item.done && <Check size={10} className="text-white" strokeWidth={3} />}
                  </div>
                  <span className={`text-sm ${item.done ? 'line-through text-muted' : 'text-navy'}`}>
                    {item.text}
                  </span>
                </button>
              ))}
        ...
        <div className="fixed inset-0 z-50 flex items-end max-w-[430px] mx-auto left-1/2 -translate-x-1/2 w-full">
          ...
          <div className="relative w-full bg-white rounded-t-3xl p-6 pb-10">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-navy font-bold text-base">メモを追加</h3>
              <button onClick={() => setShowAdd(false)}><X size={20} className="text-muted" /></button>
            </div>
            <div className="space-y-3">
              <input className="w-full bg-cream rounded-xl px-4 py-3 text-navy text-sm outline-none border border-bdr focus:border-coral" placeholder="タイトル" />
              <textarea className="w-full bg-cream rounded-xl px-4 py-3 text-navy text-sm outline-none border border-bdr focus:border-coral resize-none" rows={4} placeholder="内容を入力..." />
              <button onClick={() => setShowAdd(false)} className="w-full bg-coral text-white rounded-xl py-3 font-bold text-sm">追加する</button>
            </div>
          </div>
        </div>
      )}
...
// ─── Packing sub-screen ──────────────────────────────────────────────
function PackingScreen({ trip, mode }: { trip: Trip; mode: AppMode }) {
    ...
    <div className="px-5 py-4 space-y-4">
      ...
      <div className="bg-white rounded-2xl p-4 shadow-sm">
        <div className="flex items-center justify-between mb-2">
          <span className="text-navy text-sm font-semibold">準備の進捗</span>
          <span className="text-coral font-bold text-sm">{doneCount}/{items.length}</span>
        </div>
      ...
      {/* Shared */}
      <div>
        <p className="text-muted text-xs font-medium tracking-wide uppercase px-1 mb-2">共通</p>
        <div className="space-y-2">
          {shared.map((item) => (
            <button
              key={item.id}
              onClick={() => mode === 'host' && toggle(item.id)}
            ...
            >
              ...
              <span className={`text-sm flex-1 ${item.done ? 'line-through text-muted' : 'text-navy'}`}>
                {item.name}
              </span>
            </button>
          ))}
      ...
      {/* Personal */}
      <div>
        <p className="text-muted text-xs font-medium tracking-wide uppercase px-1 mb-2">個人</p>
        <div className="space-y-2">
          {personal.map((item) => (
            <button
              key={item.id}
            ...
            >
              ...
              {item.assignee && (
                <span className="text-muted text-xs">{item.assignee}</span>
              )}
            </button>
          ))}
        </div>
    ...
    <div className="bg-cream min-h-screen pb-24">
      ...
      <div className="bg-navy px-6 pt-14 pb-6">
        <div className="flex items-center gap-3 mb-1">
          {sub !== 'menu' && (
            <button onClick={() => setSub('menu')} className="text-white/60">
              <ArrowLeft size={20} />
            </button>
          )}
          <h2
        ...
        {sub === 'menu' && <p className="text-white/50 text-xs">フライト・宿・メモ・持ち物など</p>}
      </div>
        ...
        <div className="px-5 py-4">
          <div className="grid grid-cols-2 gap-3">
            {menuItems.map((item) => (
              <button
                key={item.id}
                onClick={() => !item.comingSoon && setSub(item.id as SubScreen)}
                className={`bg-white rounded-2xl p-4 text-left shadow-sm transition-all active:scale-95 relative overflow-hidden ${
              ...
              >
                ...
                <p className="text-muted text-[11px] mt-0.5 leading-snug">{item.desc}</p>
              </button>
            ))}
          </div>
        </div>
      ...
      {sub === 'packing' && <PackingScreen trip={trip} mode={mode} />}
    </div>
  )
}
